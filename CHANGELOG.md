...
- `AsyncConversationLike` protocol for non-blocking conversation append. Extends `ConversationLike`
  with `append_async()` method. When `Complete` receives an `AsyncConversationLike` conversation,
  it uses `append_async()` for message appends, allowing compaction strategies that involve I/O
  (e.g., LLM-based summarization) to run without blocking the event loop.
