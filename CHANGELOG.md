...
- **Breaking:** `Configurable.with_context()` now deep-merges instead of replacing.
  ...
  Leaf values are shared by reference, so cost trackers, loggers, locks, and other
  stateful objects threaded through context retain identity across derivations.
  `with_context(None)` still clears; `with_context({})` is now a no-op. The merge
  primitive is exported as `llm_saia.core.merge_context`.
