...
# Default loop config used when none provided
...
class VerbConfig:
    ...
    system: str | None
    loop: LoopConfig | None = None  # Default loop config for all verbs
...
class _Verb(ABC):
    ...
    def _get_loop_config(self, loop: LoopConfig | None) -> LoopConfig:
        """Get effective loop config: call override > instance default > global default."""
        return loop or self._config.loop or DEFAULT_LOOP
    ...
    async def _loop(
        ...
        config = self._get_loop_config(loop)
    ...
    async def _complete(self, prompt: str, loop: LoopConfig | None = None) -> str:
        """Complete with tools if available, otherwise direct."""
        if self._has_tools():
            content, _ = await self._loop(prompt, loop=loop)
            return content
        return await self._backend.complete(prompt)

    async def _complete_structured(
        self, prompt: str, schema: type[T], loop: LoopConfig | None = None
    ) -> T:
        """Complete structured with tools if available, otherwise direct."""
        if self._has_tools():
            _, result = await self._loop(prompt, loop=loop, schema=schema)
